...
public class ContactLeadServiceImpl implements ContactLeadService {
 ...
 @Override public PageResponse<ContactLeadResponse> getAll(Long companyId,String search,ContactLeadStatus status,int page,int size){ if(page<0||size<1||size>ApplicationContract.MAX_PAGE_SIZE)throw new BusinessRuleException("Geçersiz sayfalama bilgisi."); return PageResponse.from(repository.search(companyId,status,normalize(search),PageRequest.of(page,size,Sort.by(Sort.Direction.DESC,"createdAt"))),this::map); }
