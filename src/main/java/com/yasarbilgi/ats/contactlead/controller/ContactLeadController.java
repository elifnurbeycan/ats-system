...
public class ContactLeadController { private final ContactLeadService service;
 ...
 @GetMapping public ResponseEntity<ApiResponse<PageResponse<ContactLeadResponse>>> getAll(@PathVariable Long companyId,@RequestParam(required=false)String search,@RequestParam(required=false)ContactLeadStatus status,@RequestParam(defaultValue="0")int page,@RequestParam(defaultValue="20")int size){return ResponseEntity.ok(ApiResponse.success(service.getAll(companyId,search,status,page,size)));}
